...
import BaseHeaderTenant from "@/layouts/partials/HeaderTenant.vue";
...
const classContainer = computed(() => {
  return {
    ...
    "sidebar-dark":
      store.layout.sidebar && store.settings.sidebarDark && !store.settings.darkMode,
    "side-overlay-o": store.layout.sideOverlay && store.settings.sideOverlayVisible,
    "side-overlay-hover": store.layout.sideOverlay && store.settings.sideOverlayHoverable,
    "enable-page-overlay": store.layout.sideOverlay && store.settings.pageOverlay,
    "page-header-fixed": store.layout.header && store.settings.headerFixed,
    "page-header-dark":
      store.layout.header && store.settings.headerDark && !store.settings.darkMode,
...
<template>
  <div id="page-container" :class="classContainer">
    ...
    <!-- Side Overlay -->
    <BaseSideFilter v-if="store.layout.sideOverlay">
      <template #header>
    ...
    <!-- Sidebar -->
    <BaseSidebar v-if="store.layout.sidebar && !isSubdomain()">
      <template #header>
    ...
    <BaseSidebarTenant v-if="store.layout.sidebar && isSubdomain()">
      <template #header>
        <slot name="sidebar-header"></slot>
      </template>
    ...
    <!-- Header -->
    <BaseHeader v-if="store.layout.header && !isSubdomain()">
    ...
    <BaseHeaderTenant v-if="store.layout.header && isSubdomain()">
      <template #content-left>
        <slot name="header-content-left"></slot>
      </template>
