...
const form = reactive({
  email: `admin@${import.meta.env.VITE_APP_NAME}`,
  password: "123456",
});
...
async function onSubmit() {
  ...
  const res = await centralStore.login(form.email, form.password);
  if (res === 200) {
      ...
      <div class="row justify-content-center push">
        <div class="col-md-8 col-lg-6 col-xl-4">
          ...
          <BaseBlock title="Sign In" class="mb-0">
            <template #options>
              <RouterLink :to="{ name: 'landing' }" class="btn-block-option fs-sm">Forgot Password?</RouterLink>
              <RouterLink :to="{ name: 'landing' }" class="btn-block-option">
