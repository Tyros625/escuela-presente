...
const payMP = async () => {
  const { data } = await api.post(
    `/payments/preference/${studentStore.paymentType}`,
    {
      ...
      try {
        ...
        if (studentStore.paymentType === "REINSCRIPCIÓN") {
          ...
          if (!condition)
            return Swal.fire(
              "Error",
              "No hay mas grados donde reinscribirse",
              "error"
            );
        }
...
const verifyGrade = async () => {
  ...
  let siguiente = studentStore.grades.find(
    (el) => el.order === actualGrade.order + 1
  );
          ...
          <div class="row mb-5">
            <div class="col-md-12 text-center">
              ...
              <p class="fs-lg fw-medium text-muted mb-0">
                Se generará un link de pago para la adquisición de credencial de
                la escuela.
              </p>
            </div>
          </div>
